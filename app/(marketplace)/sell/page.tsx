...
const CATEGORIES = [
  'Fashion & Clothing','Food Services','Room Essentials','School Supplies',
  'Tech Gadgets','Cosmetics','Snacks','Deodorants','Books','Furniture',
]

const CAT_ICONS: Record<string, string> = {
  'Fashion & Clothing': '👔','Food Services': '🍔','Room Essentials': '🏠',
  'School Supplies': '🎒','Tech Gadgets': '🎧','Cosmetics': '💄',
  'Snacks': '🍿','Deodorants': '🌿','Books': '📚','Furniture': '🪑',
}
      ...
      <div className="scale-in relative bg-white rounded-t-3xl sm:rounded-3xl shadow-2xl w-full sm:max-w-md p-6 z-10 max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex gap-3">
          ...
          <button
            ...
            style={{ background: agreed ? 'linear-gradient(135deg, #6366f1, #4c1d95)' : undefined, backgroundColor: !agreed ? '#6366f1' : undefined }}
          >
            List Product ✨
          </button>
        </div>
...
export default function SellPage() {
    ...
    const loadAreas = async () => {
      try {
        ...
        if (cached) {
          ...
          if (areas.length > 0) { setDeliveryAreas(areas); setCheckingAuth(false); return }
        }
        const res = await fetch('/api/auth/me', { headers: { Authorization: `Bearer ${token}` } })
        if (res.ok) { const data = await res.json(); setDeliveryAreas(parseJsonArray(data?.user?.university?.deliveryAreas)) }
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    for (const file of filesToProcess) {
      if (!ALLOWED_IMAGE_TYPES.includes(file.type)) { setError(`"${file.name}" is not allowed. Only JPG, PNG, WEBP.`); continue }
      if (file.size > MAX_IMAGE_SIZE_MB * 1024 * 1024) { setError(`"${file.name}" exceeds ${MAX_IMAGE_SIZE_MB}MB.`); continue }
      const reader = new FileReader()
      reader.onloadend = async () => {
        const base64 = reader.result as string
        setImages(prev => [...prev, { preview: base64, url: '', uploading: true }])
        const cloudUrl = await uploadToCloudinary(base64)
        if (cloudUrl) setImages(prev => prev.map(img => img.preview === base64 ? { ...img, url: cloudUrl, uploading: false } : img))
        else { setImages(prev => prev.filter(img => img.preview !== base64)); setError('Failed to upload image. Try again.') }
      }
  ...
  return (
    <div className="min-h-screen bg-[#f0f2f5]">
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 py-6 pb-28 space-y-4">
        ...
        <div className="step-card bg-white rounded-2xl border border-gray-100 shadow-sm p-5 sm:p-6">
          <StepHeader num="1" title="Product Details" subtitle="Name, price, quantity and category" />
        ...
        {/* ── Step 2: Images ── */}
        <div className="step-card bg-white rounded-2xl border border-gray-100 shadow-sm p-5 sm:p-6">
          <StepHeader num="2" title="Product Photos" subtitle="Up to 3 images · JPG, PNG, WEBP · Max 8MB each" />
        ...
        {/* ── Step 3: Tags ── */}
        <div className="step-card bg-white rounded-2xl border border-gray-100 shadow-sm p-5 sm:p-6">
          <StepHeader num="3" title="Product Tags" subtitle="Describe your product with keywords — helps buyers find it" />
            ...
            <div className="flex items-center gap-2 flex-1 min-w-[140px]">
              ...
              {tagInput.trim() && (
                <button type="button" onClick={() => addTag(tagInput)}
                  className="flex items-center gap-1 px-3 py-1 text-white text-xs font-black rounded-full flex-shrink-0 transition-all hover:scale-105"
                  style={{ background: 'linear-gradient(135deg, #6366f1, #7c3aed)' }}>
          ...
          {suggestions.length > 0 && (
            <div>
              <p className="text-[11px] font-black text-gray-400 uppercase tracking-wider mb-2">Quick add:</p>
              <div className="flex flex-wrap gap-1.5">
                {suggestions.slice(0, 14).map(s => (
                  <button key={s} type="button" onClick={() => addTag(s)}
                    className="suggestion-chip px-3 py-1.5 bg-white border border-gray-200 text-gray-600 text-xs font-semibold rounded-full">
                    + {s}
                  </button>
                ))}
              </div>
          ...
          {tags.length > 0 && <p className="text-xs text-gray-400 mt-3">{tags.length} tag{tags.length !== 1 ? 's' : ''} added</p>}
        </div>

        {/* ── Step 4: Location ── */}
        <div className="step-card bg-white rounded-2xl border border-gray-100 shadow-sm p-5 sm:p-6">
          <StepHeader num="4" title="Pickup Location" subtitle="Where should the rider collect this product?" />
